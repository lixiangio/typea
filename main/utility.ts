...
import { methodKey, optionalKey, indexKey, extensionKey } from './common.js';
import type { TypeFn, StructFn, TypeObject, ExtensionNode } from './common.js';
 ...
 * 通常用于将对象和数组包装为可选
 */
export function optional(node: object | any[] | TypeFn | StructFn | TypeObject) {

  const newNode = { [indexKey]: node };
...
interface UnionType {
  [indexKey]?: this
  [Symbol.iterator]: () => any
  [methodKey]: (options: undefined, value: any) => any
}
...
export function union(...nodes: unknown[]) {
  ...
  const newNode: UnionType = {
    [methodKey](options, value: any) {
      let errorInfo = '';
      ...
      return {
        ...
        next() {
          if (this.end) {
            ...
            this.end = true;
            return { value: { [extensionKey]: newNode } };
          }
        }
      };
    },
