...
import { methodKey, optionsKey, optionalKey, extensionKey, indexKey } from './common.js';
...
export function object(node: ObjectIndex, data: any): Return {
  ...
  if (hasOwnProperty.call(node, indexKey)) {

    const indexSbuNode = node[indexKey];
...
export function array(node: any[], data: any[]): Return {
  ...
  for (const item of node) {
    ...
    if (item instanceof Object) {
      ...
      if (hasOwnProperty.call(item, extensionKey)) {

        let next = true;
        const subNode = item[extensionKey];
