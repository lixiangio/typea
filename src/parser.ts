import { type, symbols, type Methods, type Options } from './types.js';
...
export default class Parser {
   ...
   * @param node 验证表达式
   */
  recursion(key: string | number, data: any, node: any) {
   ...
   * @param node 验证表达式
   */
  object(key: string | number, data: any, node: ObjectIndex) {
    ...
    // node 为对象结构
      ...
      if (typeof data !== 'object') {
        // 宽松模式下，跳过空值
        if (this.mode === 'loose') {
          if (defaultIgnore.includes(data)) return {};
        }
   ...
   * @param express 
   */
  array(key: number | string, data: any[], express: any[]) {
